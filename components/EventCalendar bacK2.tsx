  ...
  return (
    <div className="grid grid-cols-7 gap-2">
      {Array.from({ length: 35 }).map((_, i) => (
        <div
          key={i}
          className="aspect-square w-full rounded-lg border border-gray-200 bg-gray-100 animate-pulse"
        />
      ))}
    </div>
  );
    ...
    <div className="mt-6 overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-sm">
      <div className="bg-gradient-to-r from-purple-900 to-black px-5 py-4 text-white">
        <div className="flex items-center justify-between gap-4">
          ...
          <p className="text-sm text-white/80">{venueSlug.replace(/-/g, " ")}</p>
        </div>
      ...
      <div className="p-4 md:p-6">
        ...
        <div
          key={monthKey}
          className="transition-all duration-200 ease-out animate-fade-in"
        >
          {loading ? (
            <CalendarSkeleton />
          ) : (
              ...
              {calendarCells.map((day, index) => {
                ...
                return (
                  <div
                    ...
                    className={`aspect-square w-full rounded-lg border-2 p-2 transition ${isToday
                        ...
                        {dayEvents.slice(0, 3).map((event) => {
                          ...
                          return (
                            <button
                              ...
                              className={`h-2.5 w-2.5 rounded-full transition ${isSelected
